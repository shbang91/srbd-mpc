...
void CostFunction::setQP(const ContactSchedule& contact_schedule, 
  ...
  // Change in COM Position between MPC nodes
  ...
  // Integrate velocity command and current pose to get expected COM trajectory
  // Populates base_pose_ref vector
  ...
  for (int i=0; i<qp_data.dim.N+1; ++i) {
    // Get error linearized around base_pose_ref
