...
//  There is also an optional setting to smoothen the current solution from the
//  previous solution
...
public:
  ...
  bool smooth_from_prev_result_; // Whether to smooth the solution of the MPC
                                 // iteration using the previous result
  ...
  bool rotate_inertia; // whether or not the inertia needs to be rotated to the
                       // world frame. If the inertia is expressed in body frame
                       // this needs to be true. Otherwise if the inertia is
                       // already updated to be in the world frame, this can be
                       // set to false.

  Eigen::VectorXd
      latest_f_vec_out; // Container holding the the latest computed force
                        // output for control (not to be confused with the force
                        // at the end of the horizon)
  Eigen::VectorXd
      f_prev; // Previous reaction force vector for the first horizon.

  Eigen::VectorXd X_pred; // dimension: (13*horizon) state prediction over the
                          // horizon. [x_{k+1}, x_{k+2}, ..., x_{k+horizon}]
  Eigen::VectorXd F_out;  // dimension: (n_Fr*horizon) forces to exert over the
                          // horizon. [f_{k}, f_{k+1}, ..., f_{k - 1 + horizon}]

  std::shared_ptr<GaitCycle> gait_cycle_ptr; // pointer to the gait cycle object
  std::shared_ptr<ReactionForceSchedule>
      reaction_force_schedule_ptr; // pointer to the reaction force schedule

  // Vector cost for the MPC: <<  th1,  th2,  th3,  px,  py,  pz,   w1,  w2, w3,
  // dpx,  dpy,  dpz,  g last term is gravity and should always be 0,0 cost_vec
  // << 0.25, 0.25, 10.0, 2.0, 2.0, 50.0, 0.0, 0.0, 0.30, 0.20, 0.2, 0.10, 0.0;
  ...
  Eigen::VectorXd getx0(const double roll, const double pitch, const double yaw,
                        const double com_x, const double com_y,
                        const double com_z, const double roll_rate,
                        const double pitch_rate, const double yaw_rate,
                        const double com_x_rate, const double com_y_rate,
  ...
  // Get computed ground forces in matrix form where the columns are the ground
  // reaction forces in the same order as r_feet.
  ...
  void rotateBodyInertia(bool rotate_inertia_in) {
    rotate_inertia = rotate_inertia_in;
  }

  void setRobotInertia(const Eigen::MatrixXd &inertia_in) {
    I_robot = inertia_in;
  }
  void setDt(const double mpc_dt_in) {
    mpc_dt = mpc_dt_in;
  }                                       // MPC dt interval per horizon
  void setHorizon(const int &horizon_in); // MPC horizon (number of steps)

  void setMu(const double mu_in) {
    mu = mu_in;
  }                                      // Set the coefficient of friction
  void setMaxFz(const double fz_max_in); // Set the maximum z reaction force for
                                         // one force vector
  void setSmoothFromPrevResult(const bool smooth_prev_in) {
    smooth_from_prev_result_ = smooth_prev_in;
  } // Sets whether to smoothen the current solution using the previous solution
  ...
  void setCustomReactionForceSchedule(
  ...
  // Set Terminal vector cost for the MPC: <<  th1,  th2,  th3,  px,  py,  pz,
  // w1,  w2,   w3,   dpx,  dpy,  dpz,  g use_terminal_cost_in : if true, will
  // use the terminal cost specified.
  void setTerminalCostVec(
      const bool use_terminal_cost_in,
      const Eigen::VectorXd &terminal_cost_vec_in); // Sets the cost vector.

  void setControlAlpha(const double control_alpha_in) {
    control_alpha_ = control_alpha_in;
  }
  void setDeltaSmooth(const double delta_smooth_in) {
    delta_smooth_ = delta_smooth_in;
  }

  void enableCustomSmoothing(const bool enable_custom_smoothing) {
    custom_smoothing_ = enable_custom_smoothing;
  }
  ...
  // Helper function which transforms a constant x_des to a constant trajectory
  // reference Input: x_des \in \mathbf{R}^{13} Ouput: X_ref \in
  // \mathbf{R}^{13*horizon}
  void get_constant_desired_x(const Eigen::VectorXd &x_des,
                              Eigen::VectorXd &X_ref);

  // The main solve MPC routine
  // All the values are in world frame
  // Inputs:
  // 		x0 = [Theta, p, omega, pdot, g] \in \mathbf{R}^{13}    (Starting
  // state of the system) 		X_des \in \mathbf{R}^{13*horizon}
  // (Reference or desired state evolution) 		r_feet \in \mathbf{R}^{3
  // x number_of_point_contacts} (Matrix of point contact locations where each
  // column is the xyz location of the contact) Outputs:
  //		x_pred \in \mathbf{R}^{13} - The predicted state after a time
  // interval of mpc_dt has dimension 		f_vec_out \mathbf{R}^{3 x
  // number_of_point_contacts} The xyz forces needed to be exerted at the
  // contact points to track X_des.
...
private:
  ...
  void cont_time_state_space(const Eigen::VectorXd &x_current,
                             const Eigen::MatrixXd &r_feet, Eigen::MatrixXd &A,
                             Eigen::MatrixXd &B);
  void discrete_time_state_space(const Eigen::MatrixXd &A,
                                 const Eigen::MatrixXd &B, Eigen::MatrixXd &Adt,
                                 Eigen::MatrixXd &Bdt);

  void qp_matrices(const Eigen::MatrixXd &Adt, const Eigen::MatrixXd &Bdt,
                   Eigen::MatrixXd &Aqp, Eigen::MatrixXd &Bqp);
  void get_force_constraints(const int &n_Fr, Eigen::MatrixXd &CMat,
                             Eigen::VectorXd &cvec);
  void get_qp_constraints(const Eigen::MatrixXd &CMat,
                          const Eigen::VectorXd &cvec, Eigen::MatrixXd &Cqp,
                          Eigen::VectorXd &cvec_qp);
  void get_qp_costs(const int &n, const int &m,
                    const Eigen::VectorXd &vecS_cost,
                    const double &control_alpha, Eigen::MatrixXd &Sqp,
                    Eigen::MatrixXd &Kqp, Eigen::MatrixXd &D0);

  // Converts location of the feet expressed in world frame to the CoM frame. We
  // assume the CoM orientation frame is always aligned with world.
  void convert_r_feet_to_com_frame(const Eigen::VectorXd &p_com,
                                   const Eigen::MatrixXd &r_feet,
