...
public:
  ...
  GaitCycle();
  // swing_time_in: length of flight time of a particular contact point in
  // seconds total_gait_duration_in: total time of the the gait in seconds
  // gait_offsets_in: time offsets in seconds to be used for the phase of a
  // contact point.
  GaitCycle(const double swing_time_in, const double total_gait_duration_in,
  ...
  // offset in seconds
  // Returns the gait phase value \in [0, 1) given the start time, current time,
  // and the time offset
  double getGaitPhaseValue(const double start_time, const double time,
                           const double offset);
...
private:
  int m_num_contact_points; // internal variable that keeps track of the number
                            // of contact points
  double m_time; // internal variable that keep track of the current time of the
                 // gait
  double m_start_time; // internal variable that keep track of the start time of
                       // the gait
  double m_flight_phase; // value of phase when flight occurs

  std::vector<int>
      m_internal_gait_contact_states; // a vector of the contact states
  std::vector<double> m_internal_gait_phase_states; // a vector of the phase
                                                    // states of the contacts
  std::vector<double>
      m_gait_offsets; // a vector of the offset times for the contacts
